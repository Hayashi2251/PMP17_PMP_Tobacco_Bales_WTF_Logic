codeunit 60402 "PMP17 Tobacco Bales Whse. Tf."
{
    ...
    procedure Test_InsertItemJnlLine(var tempItemJnlLine: Record "Item Journal Line" temporary; PackageNoInfoRec: Record "Package No. Information"; UserSetupRec: Record "User Setup"; TransferToBinCode: Code[50]; PostingDate: Date): Boolean
    var
        Item: Record Item;
        ...
        LastLineNo: Integer;
    begin
        PackageNoInfoRec.CalcFields(Inventory, "PMP04 Bin Code", "PMP04 Lot No.", "PMP07 Location Code");
        Item.Reset();
        IJL.Reset();
        ...
        BinContentRec.SetRange("Item No.", tempItemJnlLine."Item No.");
        BinContentRec.SetRange("Variant Code", tempItemJnlLine."Variant Code");
        BinContentRec.SetRange("Location Code", PackageNoInfoRec."PMP07 Location Code");
        BinContentRec.SetRange("Bin Code", PackageNoInfoRec."PMP04 Bin Code");
        BinContentRec.SetRange("Package No. Filter", PackageNoInfoRec."Package No.");
        BinContentRec.SetRange("Lot No. Filter", PackageNoInfoRec."PMP04 Lot No.");
        ...
        if BinContentRec.FindFirst() then begin
            BinContentRec.CalcFields(Quantity, "Quantity (Base)");
            tempItemJnlLine.Validate(Quantity, BinContentRec.Quantity);
        end else begin
            tempItemJnlLine.Validate(Quantity, PackageNoInfoRec.Inventory);
        end;
    ...
    var
        ...
        LastLineNo: Integer;
    begin
        TbcoBalesTFLine.CalcFields("Description", "Sub Merk 1", "Sub Merk 2", "Sub Merk 3", "Sub Merk 4", "Sub Merk 5", "Old Bale Position");
        Item.Reset();
        IJL.Reset();
        ItemJnlTemplate.Reset();
        ItemJnlBatch.Reset();
        ...
        tempItemJnlLine.Validate("Item No.", TbcoBalesTFLine."Item No.");
        tempItemJnlLine.Validate("Variant Code", TbcoBalesTFLine."Variant Code");
        tempItemJnlLine.Validate("Location Code", UserSetupRec."SME073 Working Location");
        tempItemJnlLine.Validate("New Location Code", UserSetupRec."SME073 Working Location");
        Item.Get(TempItemJnlLine."Item No.");
        tempItemJnlLine.Validate("Unit of Measure Code", Item."Base Unit of Measure");
        tempItemJnlLine.Validate(Quantity, TbcoBalesTFLine.Inventory);
        tempItemJnlLine.Validate("Bin Code", TbcoBalesTFLine."Curr. Bin Code");
        tempItemJnlLine.Validate("New Bin Code", TransferToBinCode);
        // tempItemJnlLine."Package No." := TbcoBalesTFLine."Package No.";
        // tempItemJnlLine."Lot No." := TbcoBalesTFLine."Lot No.";
        tempItemJnlLine.Validate("Reason Code", ExtCompanySetup."PMP17 Tobacco Tf. Reason Code");
        tempItemJnlLine."PMP15 Sub Merk 1" := TbcoBalesTFLine."Sub Merk 1";
        tempItemJnlLine."PMP15 Sub Merk 2" := TbcoBalesTFLine."Sub Merk 2";
        tempItemJnlLine."PMP15 Sub Merk 3" := TbcoBalesTFLine."Sub Merk 3";
        tempItemJnlLine."PMP15 Sub Merk 4" := TbcoBalesTFLine."Sub Merk 4";
        tempItemJnlLine."PMP15 Sub Merk 5" := TbcoBalesTFLine."Sub Merk 5";
        if tempItemJnlLine.Insert() then
            exit(true)
        else
            exit(false);
    ...
    procedure GenerateRecReserveEntryItemJnlLine(var RecItemJnlLine: Record "Item Journal Line"; var TbcoBalesTFLine: Record "PMP17 Tbcco Bales Transfer"; UserSetupRec: Record "User Setup"; TransferToBinCode: Code[50]; var TempTrackingSpecification: Record "Tracking Specification" temporary)
    var
        Item: Record Item;
        RecReservEntry: Record "Reservation Entry";
        TrackingSpecification: Record "Tracking Specification";
    ...
    begin
        ...
        TempGlobalEntrySummary.Reset();
        TempGlobalEntrySummary.SetFilter("Lot No.", RecItemJnlLine."Lot No.");
        TempGlobalEntrySummary.SetFilter("Package No.", TbcoBalesTFLine."Package No.");
        if TempGlobalEntrySummary.FindSet() then begin
            InsertReservEntryRecfromTempTrackSpecIJL(RecReservEntry, TempTrackingSpecification, RecItemJnlLine, UserSetupRec, TransferToBinCode, TempGlobalEntrySummary);
        end else begin
            PackageNoInfo.SetAutoCalcFields();
            PackageNoInfo.SetRange("Item No.", RecItemJnlLine."Item No.");
            PackageNoInfo.SetFilter("Variant Code", RecItemJnlLine."Variant Code");
            PackageNoInfo.SetFilter("Package No.", TbcoBalesTFLine."Package No.");
            PackageNoInfo.SetRange(Inventory, 0);
            if PackageNoInfo.FindFirst() then begin
                InsertReservEntryRecfromTempTrackSpecIJL(RecReservEntry, TempTrackingSpecification, RecItemJnlLine, UserSetupRec, TransferToBinCode, PackageNoInfo);
            end;
    ...
    begin
        ...
        if ItemJnlBatch.Get(ExtCompanySetup."PMP17 Int. Tf. Jnl. Tmpt. Name", ExtCompanySetup."PMP17 Int. Tf. Jnl. Batch Name") then begin
            if ItemJnlBatch."No. Series" <> '' then begin
                ItemJnlLine."Document No." := NoSeriesMgt.GetNextNo(ItemJnlBatch."No. Series", ItemJnlLine."Posting Date");
            end else begin
                ItemJnlLine."Document No." := NoSeriesMgt.GetNextNo(ExtCompanySetup."PMP17 Int. Document Tf. Nos.", ItemJnlLine."Posting Date");
            end;
        end;
        ItemJnlLine.Insert();
        //{<<<<<<<<<<<<<<<<<<<<<<<<<< PMP17 - SW - 2026/01/07- FINISH>>>>>>>>>>>>>>>>>>>>>>>>>>}
    ...
    procedure PostTobaccoBalesTransferItemReclass(var ItemJnlLine: Record "Item Journal Line"; PackageNoInfoRec: Record "Package No. Information"; UserSetupRec: Record "User Setup"; TransferToBinCode: Code[50]; PostingDate: Date): Boolean
    var
        //{<<<<<<<<<<<<<<<<<<<<<<<<<< PMP17 - PD - 2026/06/17 - START >>>>>>>>>>>>>>>>>>>>>>>>>>>}
        NextLineNo: Integer;
        //{<<<<<<<<<<<<<<<<<<<<<<<<<< PMP17 - PD - 2026/06/17 - FINISH >>>>>>>>>>>>>>>>>>>>>>>>>>}
    ...
    begin
        ...
        if PostingDate = 0D then
            Error('The Posting Date must have a value to posting tobacco bales transfer.');
        //{<<<<<<<<<<<<<<<<<<<<<<<<<< DMJ17 - SW - 2026/08/12 - FINISH >>>>>>>>>>>>>>>>>>>>>>>>>>}

        if Test_InsertItemJnlLine(tempItemJnlLine, PackageNoInfoRec, UserSetupRec, TransferToBinCode, PostingDate) then begin
            //{<<<<<<<<<<<<<<<<<<<<<<<<<< PMP17 - PD - 2026/06/17 - START >>>>>>>>>>>>>>>>>>>>>>>>>>>}
            // REMOVE:
    ...
    procedure Test_InsertItemJnlLine(var tempItemJnlLine: Record "Item Journal Line" temporary; var InternalTFRec: Record "PMP17 Tbco Internal Tansfer"; UserSetupRec: Record "User Setup"; TransferToBinCode: Code[50]; PostingDate: Date): Boolean
    var
        Item: Record Item;
        IJL: Record "Item Journal Line";
        ItemJnlTemplate: Record "Item Journal Template";
    ...
    begin
        ...
        IJL.SetRange("Journal Template Name", ExtCompanySetup."PMP17 Gen. Int. Tf. Jnl. Tmpt");
        IJL.SetRange("Journal Batch Name", ExtCompanySetup."PMP17 Gen. Int. Tf. Jnl. Batch");
        if IJL.FindLast() then
            LastLineNo := IJL."Line No.";
        ...
        ItemJnlTemplate.Get(ExtCompanySetup."PMP17 Gen. Int. Tf. Jnl. Tmpt");
        ItemJnlBatch.Get(ExtCompanySetup."PMP17 Gen. Int. Tf. Jnl. Tmpt", ExtCompanySetup."PMP17 Gen. Int. Tf. Jnl. Batch");
        ...
        tempItemJnlLine.Validate("Item No.", InternalTFRec."Item No.");
        tempItemJnlLine.Validate("Variant Code", InternalTFRec."Variant Code");
        tempItemJnlLine.Validate("Location Code", InternalTFRec."Curr. Location Code");
        tempItemJnlLine.Validate("New Location Code", InternalTFRec."Curr. Location Code");
        Item.Get(TempItemJnlLine."Item No.");
        tempItemJnlLine.Validate("Unit of Measure Code", Item."Base Unit of Measure");
        tempItemJnlLine.Validate(Quantity, InternalTFRec.Quantity);
        tempItemJnlLine.Validate("Bin Code", InternalTFRec."Curr. Bin Code");
        tempItemJnlLine.Validate("New Bin Code", TransferToBinCode);
        // tempItemJnlLine."Package No." := InternalTFRec."Package No.";
        // tempItemJnlLine."Lot No." := InternalTFRec."Lot No.";
        tempItemJnlLine.Validate("Reason Code", ExtCompanySetup."PMP17 Internal Transfer RC");
        tempItemJnlLine."PMP15 Sub Merk 1" := InternalTFRec."Sub Merk 1";
        tempItemJnlLine."PMP15 Sub Merk 2" := InternalTFRec."Sub Merk 2";
        tempItemJnlLine."PMP15 Sub Merk 3" := InternalTFRec."Sub Merk 3";
        tempItemJnlLine."PMP15 Sub Merk 4" := InternalTFRec."Sub Merk 4";
        tempItemJnlLine."PMP15 Sub Merk 5" := InternalTFRec."Sub Merk 5";
        if tempItemJnlLine.Insert() then
            exit(true)
        else
            exit(false);
    ...
    begin
        ...
        TempTrackingSpecification.Validate("Quantity (Base)", RecItemJnlLine.Quantity);
        TempTrackingSpecification.Validate("Qty. to Handle (Base)", RecItemJnlLine.Quantity);
        TempTrackingSpecification.Validate("Qty. to Invoice (Base)", RecItemJnlLine.Quantity);
        ItemTrackingLine.RegisterChange(TempTrackingSpecification, TempTrackingSpecification, ChangeType::Insert, false);
        ItemTrackingLine.GetTrackingSpec(TempTrackingSpecification);
    end;
    ...
    procedure GenerateRecReserveEntryItemJnlLine(var RecItemJnlLine: Record "Item Journal Line"; var InternalTFRec: Record "PMP17 Tbco Internal Tansfer"; UserSetupRec: Record "User Setup"; TransferToBinCode: Code[50]; var TempTrackingSpecification: Record "Tracking Specification" temporary)
    var
        Item: Record Item;
        RecReservEntry: Record "Reservation Entry";
        TrackingSpecification: Record "Tracking Specification";
    ...
    procedure PostInternalTransferItemReclass(var ItemJnlLine: Record "Item Journal Line"; var InternalTFRec: Record "PMP17 Tbco Internal Tansfer"; UserSetupRec: Record "User Setup"; TransferToBinCode: Code[50]; PostingDate: Date): Boolean
    var
        //{<<<<<<<<<<<<<<<<<<<<<<<<<< PMP17 - PD - 2026/06/17 - START >>>>>>>>>>>>>>>>>>>>>>>>>>>}
        NextLineNo: Integer;
        //{<<<<<<<<<<<<<<<<<<<<<<<<<< PMP17 - PD - 2026/06/17 - FINISH >>>>>>>>>>>>>>>>>>>>>>>>>>}
    ...
    begin
        ...
        if Test_InsertItemJnlLine(tempItemJnlLine, InternalTFRec, UserSetupRec, TransferToBinCode, PostingDate) then begin
            //{<<<<<<<<<<<<<<<<<<<<<<<<<< PMP17 - PD - 2026/06/17 - START >>>>>>>>>>>>>>>>>>>>>>>>>>>}
            /// REMOVE:
            // InsertItemJnlLinefromTemp(ItemJnlLine, tempItemJnlLine);
            InsertItemJnlLinefromTemp(ItemJnlLine, tempItemJnlLine, NextLineNo);
