pageextension 60411 "PMP17 Ext. Company Setup Ext" extends "PMP07 Extended Company Setup"
{
    ...
    // ================================================================================================================
    // 2026/04/24  SW         PMP17         PMP17 Gen. Int. Tf. Jnl. Tmpt   -                       Add control
    // 2026/04/24  SW         PMP17         PMP17 Gen. Int. Tf. Jnl. Batch  -                       Add control
    // 2026/04/24  SW         PMP17         PMP17 Internal Transfer RC      -                       Add control
    ...
    layout
    {
        addlast(General)
        {
            //{<<<<<<<<<<<<<<<<<<<<<<<<<< PMP17 - SW - 2026/04/24 - START >>>>>>>>>>>>>>>>>>>>>>>>>>}
            field("PMP17 Gen. Int. Tf. Jnl. Tmpt"; Rec."PMP17 Gen. Int. Tf. Jnl. Tmpt")
            {
                ApplicationArea = All;
                Caption = 'General Int. Tf. Jnl. Temp. Name';
                ToolTip = 'Determines the General Int. Tf. Jnl. Template Name used for internal transfer transactions in this add-on. If the template does not yet exist, the system will create it automatically.';
                trigger OnValidate()
                begin
                    CheckItemJnlBatchforDocumentNos(Rec."PMP17 Gen. Int. Tf. Jnl. Tmpt", Rec."PMP17 Gen. Int. Tf. Jnl. Batch");
                end;
            }
            field("PMP17 Gen. Int. Tf. Jnl. Batch"; Rec."PMP17 Gen. Int. Tf. Jnl. Batch")
            {
                ApplicationArea = All;
                Caption = 'General Int. Tf. Jnl. Batch Name';
                ToolTip = 'Determines the General Int. Tf. Jnl. Batch Name used for internal transfer transactions in this add-on. If the batch does not yet exist, the system will create it automatically. Ensure a No. Series is assigned to auto-generate item reclassification document numbers.';
                trigger OnValidate()
                begin
                    CheckItemJnlBatchforDocumentNos(Rec."PMP17 Int. Tf. Jnl. Tmpt. Name", Rec."PMP17 Gen. Int. Tf. Jnl. Batch");
                end;
            }
            field("PMP17 Internal Transfer RC"; Rec."PMP17 Internal Transfer RC")
            {
                ApplicationArea = All;
                Caption = 'Internal Transfer Reason Code';
                ToolTip = 'Specifies the default Reason Code used for Internal Transfer transactions.';
            }
            //{<<<<<<<<<<<<<<<<<<<<<<<<<< PMP17 - SW - 2026/04/24 - FINISH >>>>>>>>>>>>>>>>>>>>>>>>>>}
        }
        addlast(Warehouse)
        {
