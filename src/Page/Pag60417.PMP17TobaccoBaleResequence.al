                ...
                field(BinCode; BinCode)
                {
                    ...
                    begin
                        ...
                        if Page.RunModal(Page::"Bin List", BinRec) = Action::LookupOK then begin
                            ...
                            CurrentLocationCode := UserSetupRec."SME073 Working Location";
                        end;
                    end;
            ...
            group(Page02)
            {
                ...
                repeater(PkgNo__List)
                {
                    ...
                    field("Bin Code"; Rec."Bin Code")
                    {
                        ApplicationArea = All;
                        Caption = 'To Bin Code';
                        ToolTip = 'Specifies the To Bin Code associated with this line';
                        Editable = false;
                    }
    ...
    actions
    {
        area(navigation)
        {
            ...
            action(Post)
            {
                ...
                trigger OnAction()
                var
                    IsTheDataValidToDoAPosting: Boolean;
                    CountLastIRJ: Integer;
                    PkgNoInfoRec: Record "Package No. Information";
                begin
                    //{<<<<<<<<<<<<<<<<<<<<<<<<<< PMP17 - SW - 2026/06/17 - START >>>>>>>>>>>>>>>>>>>>>>>>>>}
                    Clear(IsTheDataValidToDoAPosting);

                    Rec.Reset();
                    if Rec.FindSet() then
                        repeat
                            if Rec."Curr. Bin Code" <> Rec."Bin Code" then begin
                                IsTheDataValidToDoAPosting := true;
                            end;
                        until Rec.Next() = 0;
                    //{<<<<<<<<<<<<<<<<<<<<<<<<<< PMP17 - SW - 2026/06/17 - FINISH >>>>>>>>>>>>>>>>>>>>>>>>>>}

                    if IsTheDataValidToDoAPosting then begin
                        Rec.Reset();
